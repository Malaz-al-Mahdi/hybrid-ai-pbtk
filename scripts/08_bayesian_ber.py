...
def propagate_to_ber(
    log10_clint_samples: np.ndarray,
    pilot:               pd.DataFrame,
    ...
    for i, (_, row) in enumerate(pilot.iterrows()):
        ...
        if aed_ref is not None and np.isfinite(aed_ref) and aed_ref > 0:
            ...
            aed_samples = ac50 * mw / 1e3 * cl_samples / Q_H
...
def plot_credible_intervals(result_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_ylabel("log1₀(BER)  [lower = higher concern]", fontsize=11)
    ax.set_title(
...
def plot_posterior_top5(result_df: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, (_, row) in zip(axes, df.iterrows()):
        ...
        ax.set_xlabel("log1₀(BER)", fontsize=9)
...
def plot_clint_uncertainty(
    ...
    ax.set_xlabel("log1₀(Clint_ref)  [Step 2 best estimate]", fontsize=11)
    ax.set_ylabel("log1₀(Clint_BNN) [MC Dropout posterior median]", fontsize=11)
